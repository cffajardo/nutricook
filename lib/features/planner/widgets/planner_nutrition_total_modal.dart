...
class PlannerNutritionTotalsModal extends ConsumerWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text('Daily Nutrition', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900)),
          Text('Totals for ${selectedDate.day}/${selectedDate.month}/${selectedDate.year}', 
            style: const TextStyle(color: Colors.black38, fontWeight: FontWeight.bold)),
          const SizedBox(height: 24),
        
          dailyAsync.when(
            loading: () => _buildUnifiedHero(const NutritionInfo(
              calories: 0,
              carbohydrates: 0,
              protein: 0,
              fat: 0,
            ...
            error: (error, stackTrace) => _buildUnifiedHero(const NutritionInfo(
              calories: 0,
              carbohydrates: 0,
              protein: 0,
              fat: 0,
          ...
          const Text('Meal Breakdown', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 12),
          
          Expanded(
            child: itemsAsync.when(
              loading: () => const Center(child: CircularProgressIndicator()),
              error: (error, _) => Center(child: Text('Failed to load data: $error')),
              data: (items) => ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final mealItems =
                      items.where((item) => item.mealType == meal).toList();
                  final mealNutrition =
                      calculatePlannerNutrition(plannerItems: mealItems);
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: [
            ...
            _buildDivider(),
            _buildHeroStat('Carbs', nutrition.carbohydrates.toStringAsFixed(1), 'g'),
            _buildDivider(),
            ...
            _buildDivider(),
            _buildHeroStat('Sodium', nutrition.sodium.toStringAsFixed(0), 'mg'),
          ],
        ),
      ),
    );
  ...
  Widget _buildMealRow(String meal, int calories) {
    return Container(
      ...
      child: Row(
        children: [
          Text(meal, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
          const Spacer(),
          Text('$calories kcal', style: const TextStyle(color: AppColors.rosePink, fontWeight: FontWeight.w900)),
